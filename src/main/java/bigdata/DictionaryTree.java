...
public class DictionaryTree {
    ...
    private void addStr(String word, Node node) {
        ...
        if (null != str) {
            //寻找公共前缀
            String commonPrefix = "";
            for (int i = 0; i < (word.length() > str.length() ? str.length() : word.length()); i++) {
            ...
            if (commonPrefix.length() > 0) {
                if (commonPrefix.length() == str.length() && commonPrefix.length() == word.length()) { //与之前的词重复，则取消之前node计数值的自增
                    //node.count--;
                } else if (commonPrefix.length() == str.length() && commonPrefix.length() < word.length()) {//前缀相同，余下不同
                    //剩余的串
    ...
    private int countStr(String prefix, Node node) {
        ...
        if (null != str) {
            ...
            if (prefix.indexOf(str) != 0 && str.indexOf(prefix) != 0) {
                return 0;
            } else if (str.indexOf(prefix) == 0) {//前缀匹配字符，且前缀较短
                //找到目标节点，返回单词数
                return node.count;
            } else if (prefix.indexOf(str) == 0) {//前缀匹配字符，且字符串较短
                //剩余字符串继续匹配子节点
